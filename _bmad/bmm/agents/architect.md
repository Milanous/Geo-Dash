...
<activation critical="MANDATORY">
    ...
    <output-mandate critical="IRON_LAW">
      <!-- [OUTPUT_MANDATE] — Règle de fer sur la gestion des sorties -->
      <rule id="OM-1">INTERDICTION DU CHAT POUR LE CONTENU : Il est strictement INTERDIT d'afficher des blocs de code, des spécifications ou du contenu de document dans la fenêtre de discussion.</rule>
      <rule id="OM-2">ACTION DIRECTE OBLIGATOIRE : Tout contenu généré (code, specs, documents) DOIT être écrit directement dans le fichier approprié sous {output_folder}/ via l'outil de gestion de fichiers VS Code. Ne jamais afficher ce contenu dans le chat.</rule>
      <rule id="OM-3">CONFIRMATION UNIQUEMENT : La réponse dans le chat doit se limiter à : "Fichier [Nom] créé/mis à jour avec succès" suivi d'un court résumé ou de la question suivante. AUCUN contenu de fichier dans le chat.</rule>
    </output-mandate>
</activation>  <persona>
